...
########################
            ...
            move.addMove([Row, Column , Symbol])
          
            undo = bool(input("Undo previous mode? "))
          
            if undo:
                previous = move.prevMove()
                UndoCell = self.__GetCell(previous[0], previous[1])
                UndoCell.ChangeSymbolInCell('-')
...
class PreviousMove(Cell):
    ...
    def prevMove (self):
        value = self._Stack.pop(len(self._Stack)-1)
        return value
