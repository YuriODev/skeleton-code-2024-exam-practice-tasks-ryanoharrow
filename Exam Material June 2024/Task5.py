...
############################# CHANGES START HERE #############################
                ...
                print(f"Number of symbols left: {self.__SymbolsLeft}")
        
                print()
                self.DisplayPuzzle()
                print()
                remove = bool(input("Do you wish to remove a symbol? "))
        
                if remove == True:

                    cc = int(input("Enter the row and column you wish to remove (i.e (4,5) as 45): "))
                    x = cc // 10
                    y = cc % 10

                cell = self.__GetCell(x,y)
