...
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=#
                ...
                print ("Symbols left:",self.__SymbolsLeft)
    
                remove = input("Do you  want to remove a symbol? ")

                if remove == "Yes":
                    row = int(input("Enter row to delete from:" ))
                    column = int(input("Enter column to delete from:" ))

                    cell = self.__GetCell(row,column)

                    if cell.IsEmpty():
                        print("Location is empty")
                    elif cell.CheckSymbolAllowed(cell.GetSymbol()) == False:
                        print("Cannot remove symbol from a pre-existing pattern")
                    elif cell.GetSymbol() == "@":
                        print("Blocked cells cannot be changed" )
                    else:
